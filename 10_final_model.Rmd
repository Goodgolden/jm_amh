...
***Notice: ask EJC for which computer to use for long chain running***
...
```{r}
xtable::xtable(tbl_auc, caption = "AUC Table")
```

The ROC curves for the joint model for $Tstart = 2, 3, 4, 5, 6, 8$ by rows,
...
names(tbl_bs) <- c("Dt1", "Dt2", "Dt3", "Dt4", "Dt5")

tbl_bs %>% knitr::kable()
```

```{r}
xtable::xtable(tbl_bs, caption = "BS Table")
```
