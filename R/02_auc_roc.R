...
tvROC.jm <- function (object,
  ...
  # pp <- preds[["mcmc"]] %>%
  #   as.data.frame()
  ## to the end we know that the auc for the mean
  ## behaves different from the auc for each iteration
  ## }}}------------------------------------------------------------------------
  qi_u_t <- 1 - preds$pred
  ...
  # auc_ci.bayes <- auRoc::auc.para.bayes(qi_u_t, ind, conf.level = 0.95,
  #                # dist=c("normalDV", "normalEV", "exponential"),
  #                nburn = 1000, niter = 10000, nthin = 10, seed = 100)

  auc_ci.freq <- map_dfc(c(0.5, 0.9, 0.95),
                         ~auRoc::auc.para.frequentist(qi_u_t,
                                                      ind,
                                                      conf.level = .x,
                        # dist=c("normalDV", "normalEV", "exponential"),
                        method = "wald") %>%
  ...
  auc_ci <- unlist(auc_ci.freq) %>% unique() %>% as.numeric()

  names(auc_ci) <- c("0.5", "0.025", "0.25", "0.75", "0.9", "0.05", "0.95")

 ## I will keep the preds_roc here for future
 ## investigation but this is wrong for the auc ci
 ## in this amh practice for rate-value model
  # preds_roc <- function(pp1,
  #                       ...) {
  #   qi_u_t <- 1 - pp1
  #   names(qi_u_t) <- preds$id
  #   qi_u_t <- qi_u_t[preds$times > Tstart]
  ...
  # rocs <- map(pp[500:1000], preds_roc)

  return(auc_ci)
}


tvAUC <- function (object, newdata, Tstart, iter, ...) {
  UseMethod("tvAUC")
}
...
tvAUC_CI <- function (object,
  ...
  pp <- cbind(prediction.jm[["pred"]],
              prediction.jm[["mcmc"]])%>%
    as.data.frame()

  # pp1 <- prediction.jm[["mcmc"]] %>% rowMeans() %>%
  #   cbind(prediction.jm[["pred"]])
  #
  # View(pp1)
  ## }}}------------------------------------------------------------------------
