...
\begin{split}
  ...
  & \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \  \ \ \ \ \ \  random = time| id) \\
  surv18: \ \ \ & Surv(etime, event) \sim  factor(bmi) + site + ethnic + smoke \\
...
jmb2.118.value.rate <- JMbayes2::jm(Surv_object = surv18,
                                  Mixed_objects = lme1, 
                                  functional_forms = list("lamh" = ~ value(lamh) + slope(lamh)),
                                  time_var = "time",
                                  control = jmcontrol) 

summary118.value.rate <- summary(jmb2.118.value.rate)$Survival 

save(jmb2.118.value.rate,
     file = "results/jmbayes2_118_2M.Rdata")
...
```{r}
load("results/jmbayes2_118_2M.Rdata")

summary118.value.rate 
```

### SD Calculation posterior

```{r}
random_intercept <- cbind(jmb2.118.value.rate$mcmc$b[[1]][, 1, ]) 
                          # jmb2.118.value.rate$mcmc$b[[2]][, 1, ], 
                          # jmb2.118.value.rate$mcmc$b[[3]][, 1, ], 
                          # jmb2.118.value.rate$mcmc$b[[4]][, 1, ])
fixed_intercept <- cbind(t(jmb2.118.value.rate$mcmc$betas1[[1]][, 1]))
                         # t(jmb2.118.value.rate$mcmc$betas1[[2]][, 1]), 
                         # t(jmb2.118.value.rate$mcmc$betas1[[3]][, 1]), 
                         # t(jmb2.118.value.rate$mcmc$betas1[[4]][, 1]))

# random_intercept <- random[, 1, ] 

random_slope <- cbind(jmb2.118.value.rate$mcmc$b[[1]][, 2, ])
                      # jmb2.118.value.rate$mcmc$b[[2]][, 2, ], 
                      # jmb2.118.value.rate$mcmc$b[[3]][, 2, ], 
                      # jmb2.118.value.rate$mcmc$b[[4]][, 2, ])
fixed_slope <- cbind(t(jmb2.118.value.rate$mcmc$betas1[[1]][, 2]))
                     # t(jmb2.118.value.rate$mcmc$betas1[[2]][, 2]), 
                     # t(jmb2.118.value.rate$mcmc$betas1[[3]][, 2]), 
                     # t(jmb2.118.value.rate$mcmc$betas1[[4]][, 2]))
...
-   The fixed effect slope term $\sigma_{b_1}$ = `r summary(jmb2.118.value.rate)$Outcome[2, 2]`;

-   We have the pooled $\sigma_{b1 + \beta1}$ = `r sqrt(summary(jmb2.118.value.rate)$Outcome[2, 2]^2 + mean(random_slope_sd)^2)`.

-   The 95% CI for the slope term is (`r summary(jmb2.118.value.rate)$Outcome[2, 1]- 1.96 * sqrt(summary(jmb2.118.value.rate)$Outcome[2, 2]^2 + mean(random_slope_sd)^2)`, `r summary(jmb2.118.value.rate)$Outcome[2, 1] + 1.96 * sqrt(summary(jmb2.118.value.rate)$Outcome[2, 2]^2 + mean(random_slope_sd)^2)`)
...
AUC2  <- map(c(1, 2, 3, 4, 5), 
                 ~try(JMbayes2::tvAUC(jmb2.118.value.rate,
                                      swan_amh03,
                                      Tstart = 2,
                                      Dt = .x)))
...
save(AUC2, AUC3, AUC4, AUC5, AUC6, AUC8, tbl_auc_value_rate,
     file = "results/jmbayes2_118_value_rate_auc_only.RData")

```

```{r}
load("results/jmbayes2_118_value_rate_auc_only.RData")
...
Brier2  <- map(c(1, 2, 3, 4, 5), 
                 ~try(JMbayes2::tvBrier(jmb2.118.value.rate,
                                      swan_amh03,
                                      Tstart = 2,
                                      Dt = .x)))
...
save(Brier2, Brier3, Brier4, Brier5, Brier6, Brier8, tbl_bs_value_rate,
     file = "results/jmbayes2_118_value_rate_brier.RData")
```

```{r}
#| label: "bs_value_rate"

load("results/jmbayes2_118_value_rate_brier.RData")
...
#| include: true
AUC2  <- map(c(1, 2, 3, 4, 5), 
             ~try(tvAUC_CI(jmb2.118.value.rate,
                           swan_amh03,
                           Tstart = 2,
                           Dt = .x)))
AUC3  <- map(c(1, 2, 3, 4, 5), 
...
save(AUC2, AUC3, AUC4, AUC5, AUC6, AUC8,
     file = "results/jmbayes2_118_value_rate_auc_ci.RData")
```

## AUC CI

```{r}
load("results/jmbayes2_118_value_rate_auc_ci.RData")
...
## Comparison for `jm117.value.rate` to `jm118.value.rate`

```{r fig.show="hold", out.width="100%"}
#| layout-ncol: 2
knitr::include_graphics("figure/Q16_jm117_value_rate_auc_plot.png")
knitr::include_graphics("figure/Q18_jm118_value_rate_auc_plot.png")
```
